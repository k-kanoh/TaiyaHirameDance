...
{
    public class SnapshotExport : ExcelUtil
    {
        ...
        public static SnapshotExport Create()
        {
            return new SnapshotExport() { package = new ExcelPackage() };
        }
        ...
        private void Export(DapperQueryContainer item)
        {
            ...
            var curr = top.Right().Bottom();
            for (int i = 0; i < fieldCount; i++)
            {
                var next = curr.Right(i);
                next.Value = item.ColumnName[i];
            }
            SetStyleColumnName(fieldCount);
            ...
            SetStyleLogicalName(fieldCount);

            curr = curr.Bottom();
            if (item.Values.Values.Any())
            {
                var i = 0;
                foreach (var data in item.Values.Values)
                {
                    ...
                    var range = curr.MultipleRange(1, fieldCount);
                    SetColorIfCounterIsOddNumberElseWhite(range, i++, "#FFF2CC");
                ...
                SetStyleValues(item.Values.Values.Count, fieldCount);
            }
            else
            {
                curr.Value = "(データが見つかりませんでした)";
                curr.Style.Font.SetFromFont(new Font("ＭＳ Ｐゴシック", 10));
            }

            SetBorder(item.Values.Values.Count, fieldCount);
        ...
        private void SetStyleValues(int rowCount, int colCount)
        {
            ...
            for (int i = 0; i < colCount; i++)
                top.Right(i).AutoFitIfGreater(30);
        }
