        ...
        private void Export(DapperQueryContainer item)
        {
            ...
            if (item.Values.Any())
            {
                ...
                foreach (var row in item.Values)
                {
                    ...
                    curr.MultipleRange(1, fieldCount).SetBackgroundColor(i++, "#FFF2CC", ExcelFillMode.AlternateRows);
                ...
                SetStyleValues(item.Values.Count, fieldCount);
            }
            else
            {
            ...
            SetBorder(item.Values.Count, fieldCount);
